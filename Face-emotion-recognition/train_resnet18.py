...
import cv2
import image_utils
from model.Res18Feature import Res18Feature,initialize_weight_goog
...
class FaceDataset(Dataset):
    def __init__(self, phase , file_list = None, transform=None , basic_aug = False):
    ...
    def __getitem__(self, idx):
        ...
        return image, torch.from_numpy(label_idx) ,torch.tensor(idx)


train_data = FaceDataset("train", train_list, transform=train_transforms, basic_aug = True)
valid_data = FaceDataset("val", valid_list, transform=val_transforms)
...
network = Res18Feature(pretrained = imagenet_pretrained) 
if not imagenet_pretrained:
    for m in network.modules():
        initialize_weight_goog(m)
network.to(device)
print('Total params: %.2fM' % (sum(p.numel() for p in network.parameters()) / 1000000.0))
...
def train(nEpochs):
  ...
  for epoch in range(nEpochs):
    ...
    for inputs, labels ,indexes in tqdm(train_loader):
        ...
        attention_weights, outputs = network(inputs)
        ...
        # outputs = 
        loss = F.cross_entropy(outputs, labels.long())+ RR_loss 
...
# Early stopping
    ...
    epoch_loss = running_val_loss / len(valid_loader.dataset)
    epoch_acc = running_val_corrects.double() / len(valid_loader.dataset)


    
    writer.add_scalar('data/val_loss_epoch', epoch_loss, epoch)
    writer.add_scalar('data/val_acc_epoch', epoch_acc, epoch)
    print("[val] Epoch: {}/{} Loss: {} Acc: {}".format( epoch+1, nEpochs, epoch_loss, epoch_acc))
    stop_time = timeit.default_timer()
    print("Execution time: " + str(stop_time - start_time))    
    if epoch_acc > acc :
        acc = epoch_acc
        print(f"*********************************************************************best val acc = {acc}")
        print("model saveing .." +"\n")
        torch.save(network.state_dict(), '/media/disk3/yrq/contest/Face-emotion-recognition/model3.pth')
        torch.save(optimizer.state_dict(), '/media/disk3/yrq/contest/Face-emotion-recognition/optimizer3.pth')
    else:
        print("\n")
